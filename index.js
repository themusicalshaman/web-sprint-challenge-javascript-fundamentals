...
// Because internal is located inside the {}
...
/* Create a function called `summation` that accepts a parameter and uses a counter to return the summation of that number. For example, `summation(4)` should return 10 because 1+2+3+4 is 10. Note, you may use a for loop for this function if you wish */
          ...
          function summation(value){
            let sumval =0;
              for (let i = 1; i <= value; i++){sumval += i;
          }
...
const zooAnimals = [
  ...
  */
      const displayNames = [];
        ...
        displayNames.push(a.animal_name + ", " + a.scientific_name)
      });
  ...
  */
          const lowCaseAnimalNames =
            zooAnimals.map(a => a.animal_name.toLowerCase());
  ...
  */
          const lowPopulationAnimals = 0;
            zooAnimals.filter(a => a.population < 5)
 
          console.log(zooAnimals.filter(a => a.population < 5));
  ...
  */
        let populationTotal = zooAnimals.reduce((acc, a) => {
          return acc + a.population;
        }, 0);
